...
makeWall :: Game -> Game
makeWall game
            |(isObject newCoord wallsCoords)=game{currMap=currMap_{previousMap=Just currMap_
                                                                    ,walls=filter (\(a, b) -> not (a == fst newCoord && b == snd newCoord)) wallsCoords
                                                                    }}

            |(not (isObject newCoord targetsCoords) && not (isObject newCoord boxesCoords))=game{currMap=currMap_{previousMap=Just currMap_
                                            ,walls=newCoord:wallsCoords
                            }}
            |otherwise = game

                where
                    currMap_        = (currMap game)
                    wallsCoords      = (walls   currMap_)
                    targetsCoords      = (targets currMap_)
                    boxesCoords      = (boxes currMap_)
                    newCoord = (player currMap_)


makeBox :: Game -> Game
makeBox game
            |(isObject newCoord boxesCoords)=game{currMap=currMap_{previousMap=Just currMap_
                                                                    ,boxes=filter (\(a, b) -> not (a == fst newCoord && b == snd newCoord)) boxesCoords
                                                                    }}

            |(not (isObject newCoord targetsCoords) && not (isObject newCoord wallsCoords))=game{currMap=currMap_{previousMap=Just currMap_
                                            ,boxes=newCoord:boxesCoords
                            }}
            |otherwise = game
...
setTarget :: Game -> Game
setTarget game
            |(isObject newCoord targetsCoords)=game{currMap=currMap_{previousMap=Just currMap_
                                                                    ,targets=filter (\(a, b) -> not (a == fst newCoord && b == snd newCoord)) targetsCoords
                                                                    }}

            |(not (isObject newCoord wallsCoords) && not (isObject newCoord boxesCoords))=game{currMap=currMap_{previousMap=Just currMap_
                                            ,targets=newCoord:targetsCoords
                            }}
...
setPlayer :: Game -> Game
setPlayer game|(not (isObject newCoord targetsCoords) && not (isObject newCoord boxesCoords) && not (isObject newCoord wallsCoords)) = game{currMap=currMap_{previousMap=Just currMap_
                                        ,player_start=(player currMap_)
                        }}
                |otherwise=game
                    where
                        currMap_ = (currMap game)
                        ...
                        newCoord = (player currMap_)
clearMap :: Game -> Game
clearMap game = game{currMap=currMap_{previousMap=Just currMap_
                                        ,walls=[]
                                        ,targets   = []
                                        ,boxes     = []
                                    }
...
handleKeys (EventKey (Char c) Down _ _) game = case c of
  'w' -> makeWall game
  'a' -> makeBox game
  'd' -> setTarget game --dot
  's' -> setPlayer game --start
  'z' -> makeStepBack game
  'r' -> startFromBegin game
  'c' -> clearMap game
  '=' -> scaleIn game
  '-' -> scaleOut game
  ...
  _ -> game
handleKeys (EventKey (SpecialKey c) Down _ _) game = case c of
    KeyDown -> moveDown game
    KeyUp -> moveUp game
    KeyLeft -> moveLeft game
    KeyRight -> moveRight game
    -- Do nothing for all other events.
    _ -> game
...
action game newCoord followCoord
            = game {currMap = mapStruct {player = newCoord
